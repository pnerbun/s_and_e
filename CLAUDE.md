...
| `botanical-motif-gold.png` | `flower.png` | Decorative flourish, used **5×** on the homepage (see the motif system below). Transparent, tinted `--gold`, generated by stripping the white bg (alpha = inverted luminance) and auto-cropping — so it drops onto light *or* dark sections. Decorative only: empty `alt` + `aria-hidden`. |

### Botanical motif system (`styles.css`)

Add `.has-botanical` to a `<section>`, then drop in:

```html
<img class="botanical left|right on-light|on-dark" src="photos/botanical-motif-gold.png"
     alt="" aria-hidden="true" loading="lazy">
```

- **Sides alternate down the page** — Our Story `right` → Services `left` → Now Leasing `right` → The Space `left` → Contact CTA `right`. `.left` is mirrored (`scaleX(-1)`) so the two sides aren't identical.
- **Opacity 35%** (30% mobile). Gold needs *opposite* treatment per background, handled by CSS `filter` rather than shipping a second asset: `.on-light` uses `brightness(0.68)` to deepen it against cream; `.on-dark` uses `brightness(1.15)` to lift it against forest/black.
- `.has-botanical` sets `position:relative; overflow:hidden`, and its `> .section-wrap` gets `z-index:1` so content stays above the flourish.
...
Local landing pages targeting "hair salon <city> WI" for the Lake Country towns the salon draws from — **Oconomowoc, Hartland, Pewaukee, Waukesha** (Delafield itself is the main site). Path: `areas/hair-salon-<city>-wi.html`.

- Generated by `gen_areas.py` (kept in the session scratchpad, not the repo) — a template for the shared chrome (nav/footer, `../` paths, preloads) + a per-town dict of **unique** prose (intro, "why," FAQ, drive directions). **Each page must stay genuinely distinct** — near-duplicate location pages are treated as doorway/spam pages by Google. If adding towns, write real local content, don't swap the city name.
- Each carries `HairSalon` (address stays Delafield, `areaServed` = that city) + `BreadcrumbList` + `FAQPage` JSON-LD.
- Internal links: the **contact page** `#areas` section links all four; each area page cross-links the others; all four are in `sitemap.xml`.
- ⚠️ Do **not** invent a physical address in these towns — the business has one location (Delafield). Pages say "a short drive from <town>," never imply a salon there.
